...
public class CreateNewLevel : EditorWindow
{
    protected string m_NewSceneName;

    protected readonly GUIContent m_NameContent = new GUIContent("New Scene Name");

    [MenuItem("PiL Tools/Create New Scene...", priority = 100)]
    static void Init()
    {
        CreateNewLevel window = GetWindow<CreateNewLevel>();
        window.Show();
    }
    ...
    {
        string[] result = AssetDatabase.FindAssets("_start");

        if (result.Length > 0)
        {
            string newScenePath = "Assets/My Levels/" + m_NewSceneName + ".unity";
        ...
        else
        {
            ...
            EditorUtility.DisplayDialog("Error",
                "The scene <_start> was not found in Assets/Scenes folder. This scene is required by the New Scene Creator.",
                "OK");
        }
    }
